...
# 📘 MySQL 事务一致性与 MVCC 行为详解[#](#-mysql-事务一致性与-mvcc-行为详解)
...
## 一、前提知识：MySQL InnoDB 的事务机制[#](#一前提知识mysql-innodb-的事务机制)

### 1.1 默认隔离级别：REPEATABLE READ[#](#11-默认隔离级别repeatable-read)
...
### 3.1 SELECT ... FOR UPDATE 会加什么锁？[#](#31-select--for-update-会加什么锁)
...
## 六、拓展：如何让事务看到最新数据？[#](#六拓展如何让事务看到最新数据)
